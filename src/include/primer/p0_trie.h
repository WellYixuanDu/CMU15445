...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (children_.find(key_char) != children_.end()) {
      LOG_INFO("The child's key_char: %c has existed \n", key_char);
      return nullptr;
    }
    if (child->key_char_ != key_char) {
      LOG_INFO("The child's key_char: %c is different than key_char: %c \n", child->key_char_, key_char);
      return nullptr;
    }
    LOG_INFO("succeed insert: %c\n", key_char);
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    if (children_.find(key_char) == children_.end()) {
      LOG_INFO("The child: %c is not exist \n", key_char);
      return nullptr;
    }
    return &children_.at(key_char);
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *tmp_node = &root_;
    for (int i = 0; i < key_size; ++i) {
      char v = key[i];
      // 终端节点
      if (key_size == i + 1) {
        if ((*tmp_node)->HasChild(v)) {
          auto child_node = (*tmp_node)->GetChildNode(v);
          if (child_node == nullptr) {
            LOG_INFO("The node: %c has child, but can't get the child node \n", v);
            latch_.WUnlock();
            return false;
          }
          if ((*child_node)->IsEndNode()) {
            // 重复键
            LOG_INFO("The key: %s have existed \n", key.c_str());
            latch_.WUnlock();
            return false;
          }
          auto new_node = std::make_unique<TrieNodeWithValue<T>>(std::move(*(child_node->get())), value);
          (*tmp_node)->RemoveChildNode(v);
          (*tmp_node)->InsertChildNode(v, std::move(new_node));
          latch_.WUnlock();
          return true;
        }
        auto tmp_children = std::make_unique<TrieNodeWithValue<T>>(v, value);
        (*tmp_node)->InsertChildNode(v, std::move(tmp_children));
        latch_.WUnlock();
        return true;
      }
      if ((*tmp_node)->HasChild(v)) {
        tmp_node = (*tmp_node)->GetChildNode(v);
      } else {
        auto tmp_new_children = std::make_unique<TrieNode>(v);
        tmp_node = (*tmp_node)->InsertChildNode(v, std::move(tmp_new_children));
      }
    }
    latch_.WUnlock();
    return false;
  ...
  bool Remove(const std::string &key) {
    ...
    int key_size = key.size();
    std::unique_ptr<TrieNode> *tmp_node = &root_;
    std::vector<std::unique_ptr<TrieNode> *> tmp_vec;
    tmp_vec.emplace_back(tmp_node);
    for (int i = 0; i < key_size; ++i) {
      char v = key[i];
      if (i == key_size - 1) {
        if ((*tmp_node)->HasChild(v)) {
          auto last_node = (*tmp_node)->GetChildNode(v);
          (*last_node)->SetEndNode(false);
          if (!(*last_node)->HasChildren()) {
            while (true) {
              auto parent_node = tmp_vec.back();
              tmp_vec.pop_back();
              char del_val = (*last_node)->GetKeyChar();
              (*parent_node)->RemoveChildNode(del_val);
      ...
      if ((*tmp_node)->HasChild(v)) {
        tmp_vec.emplace_back((*tmp_node)->GetChildNode(v));
        tmp_node = (*tmp_node)->GetChildNode(v);
      } else {
        latch_.WUnlock();
        return false;
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    for (int i = 0; i < key_size; ++i) {
      ...
      if (key_size == i + 1) {
        if ((*tmp_node)->HasChild(v)) {
          auto final_val = (*tmp_node)->GetChildNode(v);
          if ((*final_val)->IsEndNode()) {
            auto val = dynamic_cast<TrieNodeWithValue<T> *>((*final_val).get());
            *success = true;
            auto res = val->GetValue();
            latch_.RUnlock();
            return res;
          }
          *success = false;
